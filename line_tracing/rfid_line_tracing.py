...
class ColorDepthTracker(Node):
    def __init__(self):
        ...
        # 이미지 데이터를 저장할 큐
        self.image_queue = queue.Queue(maxsize=2) # 최신 이미지만 유지 (오래된 이미지 버림)
        ...
        self.start_rfid_process()  # 프로세스 기반 RFID 시작
    ...
    def start_rfid_process(self):
        self.rfid_proc = subprocess.Popen(
            ["python3", "rfid_reader_process.py"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            bufsize=1
        )
        self.rfid_thread = threading.Thread(target=self.rfid_process_loop, daemon=True)
        self.rfid_thread.start()

    def rfid_process_loop(self):
        while True:
            line = self.rfid_proc.stdout.readline()
            if not line:
                break
            uid = line.strip()
            if uid:
                self.get_logger().info(f"RFID UID (from process): {uid}")
