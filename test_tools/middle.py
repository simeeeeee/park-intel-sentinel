...
class LaneDetector(object):
    def __init__(self, color):
        ...
        if os.environ.get('DEPTH_CAMERA_TYPE') == 'ascamera':
            ...
            self.rois = (
                (380, 480, 0, 640, 0.7),  # 인덱스 0: 가장 하단 (가장 중요)
                (300, 370, 0, 640, 0.2),  # 인덱스 1: 중간
                (200, 290, 0, 640, 0.1)   # 인덱스 2: 상단 (가장 가중치 낮음)
            )
        else:
            # 일반 카메라에 맞는 ROI 예시
            self.rois = (
                (450, 480, 0, 640, 0.7),  # 인덱스 0: 가장 하단 (가장 중요)
                (390, 420, 0, 640, 0.2),  # 인덱스 1: 중간
                (330, 360, 0, 640, 0.1)   # 인덱스 2: 상단 (가장 가중치 낮음)
            )
    ...
    def __call__(self, image): 
        ...
        # 개별 ROI에서 감지된 중심점들을 저장할 리스트
        detected_cx_per_roi = [] 
        
        # 각 ROI에 대해 차선 중심 찾기
        for idx, (roi_y_start, roi_y_end, roi_x_start, roi_x_end, weight) in enumerate(self.rois):
            # ROI 영역 유효성 검사 및 클리핑
            ...
            if roi_y_start >= roi_y_end or roi_x_start >= roi_x_end:
                detected_cx_per_roi.append(-1) # 유효하지 않은 ROI는 -1 추가
                continue
            ...
            cx_on_full_image = -1 # 현재 ROI에서 감지된 X 좌표 (원본 이미지 기준)
            if len(contours) > 0: 
            ...
            detected_cx_per_roi.append(cx_on_full_image) # 감지된 X 좌표 저장
        
        # --- 최종 final_cx 계산 로직 변경 ---
        final_cx = -1
        # 가장 위쪽 ROI (인덱스 2)의 감지 결과만 사용
        top_roi_index = 2 # self.rois 튜플의 세 번째 (가장 높은) ROI
        if len(detected_cx_per_roi) > top_roi_index and detected_cx_per_roi[top_roi_index] != -1:
            final_cx = detected_cx_per_roi[top_roi_index]
            print(f"Using top ROI center X: {final_cx}") # 디버깅을 위한 출력
        else:
            # 만약 가장 위쪽 ROI에서 차선이 감지되지 않으면, 
            # 그 다음 ROI (인덱스 1)의 값을 사용하거나, 
            # 아니면 이전처럼 가중 평균을 사용하거나, -1을 반환하는 등 전략 필요
            # 여기서는 단순히 -1을 유지하거나, 아래 가중 평균 방식을 주석 해제하여 사용
            print("Top ROI did not detect lane. Using weighted average as fallback.")
            
            # (대안: 가장 위쪽 ROI가 감지되지 않았을 때 기존 가중 평균 방식 사용)
            centroid_x_sum = 0
            total_valid_weights = 0
            for idx, cx in enumerate(detected_cx_per_roi):
                if cx != -1:
                    centroid_x_sum += cx * self.rois[idx][-1]
                    total_valid_weights += self.rois[idx][-1]
            
            if total_valid_weights > 0:
                final_cx = int(centroid_x_sum / total_valid_weights)
            else:
                final_cx = -1 # 모든 ROI에서 감지 실패
        ...
        return result_image, None, final_cx
...
def main():
    ...
    while rclpy.ok() and running: # rclpy가 실행 중이고, 프로그램이 종료되지 않았을 때만 루프 실행
        ...
        processed_image, _, final_detected_cx = lane_detector_instance(image_from_queue) 
