...
def capture_and_infer(tsanpr, frame_left, frame_right, sensor_name):
    ...
    # 로컬 저장 대신 서버 전송 함수 호출
    send_results_to_server(left_results, right_results, sensor_name=sensor_name, server_url="https://222.234.38.97:8443/api/robot/status")

    end_time = time.time()
    logging.info(f"총 추론 소요 시간: {end_time - start_time:.4f} 초")

def capture_and_infer_loop(tsanpr):
    cap_left = cv2.VideoCapture("/dev/video1")
    cap_right = cv2.VideoCapture("/dev/video4")
